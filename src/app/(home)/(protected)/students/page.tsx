...
export default async function Students({ searchParams }: StudentsProps) {
    ...
    const response = await getStudents({
        search: searchParams?.name || searchParams?.email || "",
                    ...
                    <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-6">
                        {response.data.students.map((student) => {
                            ...
                            return (
                                <div
                                    key={publicId}
                                    className="relative flex flex-col items-center rounded-2.5xl sm:w-[25rem] w-[18.75rem] mx-auto p-4 bg-white bg-clip-border shadow-2xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:!shadow-none">
