...
export default function SubmitButton({ children, pending, ...props }: Props) {
  ...
  return (
    <Button
      disabled={pending}
      {...props}
      className={cn(props.className, " dark:bg-navy-600 dark:hover:bg-navy-700 dark:active:bg-navy-700 bg-brand-300 hover:bg-brand-300 active:bg-brand-400 disabled:cursor-wait text-white")}
    >
      <ImSpinner9
        size={25}
